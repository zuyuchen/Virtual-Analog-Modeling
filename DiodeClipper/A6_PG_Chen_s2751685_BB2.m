...
method = 'damped';                      % Newton's Method version
asymmetric = true;                      % Assymetric Clipping flag
L = -2;                                 % lower boundary of p in the look-up table
U = 2;                                  % upper boundary of p in the look-up table
N = 1000;                               % length of the look-up table (number of p-v pairs)
...
%
pax = linspace(L, U, N);           % p-axis (array of p values)
vax = 0*pax;                       % v-axis (array to store solutions)
for n = 1:length(pax)
    [v, guesses, residual, iter] = newtonRaphsonMethod(pax(n), method, g, dg_dv);
    vax(n) = v;
end
...
%% Cubic Lagrange Interpolation function
%
...
for n = 1:Nf-1
    ...
    switch mode
        ...
        case 'cubic'
            ...
            [val, ind] = min(abs(p - pax)); % position of the nearest p stored in the p-v table
            if val == 0
                v = vax(ind);
                break
            end
            ...
            p_ind = ((p - L)/(U-L)) * N;          % index of the p in "p-v look-up table" (in the range of 1:N)
...
if plotting
    ...
    xlim([1 iter])
    title(sprintf("%s method: g vs v", method))
...
if plotting
    ...
    ylabel('$\\Voltage(v)$', 'Interpreter', 'latex')
    if asymmetric
        title(sprintf('Assymetric Clipping of Normalized Input'))
    else
        title(sprintf('Symmetric Clipping of Normalized Input'))
    end
end
